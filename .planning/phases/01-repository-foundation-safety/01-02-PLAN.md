...
must_haves:
  truths:
    ...
    - "Secrets are extracted from .bashrc/.zshrc/.profile into ~/.secrets.env AND the inline export lines are removed from those shell configs"
    - "Shell configs source ~/.secrets.env so secrets remain available at runtime"
  ...
  key_links:
    ...
    - from: "~/.bashrc"
      to: "~/.secrets.env"
      via: "source command"
      pattern: "source.*secrets\\.env"
---

<objective>
Set up safety guardrails (gitignore, chezmoiignore, pre-commit secret scanning) and extract inline secrets from shell configs into an age-encrypted .secrets.env file managed by chezmoi. Remove inline secret exports from shell configs and replace with sourcing of ~/.secrets.env.

Purpose: Safety guardrails must be in place BEFORE any commits to prevent accidental secret exposure. Secrets must be extracted from shell configs (where they currently live as plaintext) into encrypted storage, and the original inline exports must be removed to satisfy SEC-01 ("All inline secrets removed from shell config files").
Output: Pre-commit hooks active, .gitignore/.chezmoiignore configured, secrets extracted and encrypted, inline exports removed from shell configs, shell configs source ~/.secrets.env, .secrets.env.example template committed.
</objective>
...
<task type="auto">
  <name>Task 2: Extract secrets, remove inline exports, and create encrypted .secrets.env</name>
  <files>
    ~/.secrets.env
    ~/.local/share/chezmoi/encrypted_dot_secrets.env.age
    ~/.local/share/chezmoi/.secrets.env.example
  ...
  <action>
    ...
    3. CRITICAL (SEC-01): Remove the inline secret export lines from the source shell configs.
       For each of ~/.bashrc, ~/.zshrc, ~/.profile:
       - Remove any line that exports GITHUB_PERSONAL_ACCESS_TOKEN, EXA_API_KEY, or GREPTILE_API_KEY
       - This includes `export VARNAME=...` and plain `VARNAME=...` assignment lines for these variables
       - Be careful to ONLY remove the secret export lines, not surrounding code
       - Before modifying each file, read it fully to understand context around the secret lines

       Example using sed (adapt as needed for each file):
       ```bash
       # For .bashrc - remove inline secret exports
       sed -i '/^export GITHUB_PERSONAL_ACCESS_TOKEN=/d' ~/.bashrc
       sed -i '/^export EXA_API_KEY=/d' ~/.bashrc
       sed -i '/^export GREPTILE_API_KEY=/d' ~/.bashrc

       # For .zshrc - remove inline secret exports
       sed -i '/^export EXA_API_KEY=/d' ~/.zshrc

       # For .profile - remove inline secret exports
       sed -i '/^export GITHUB_PERSONAL_ACCESS_TOKEN=/d' ~/.profile
       ```
       Adjust patterns if the lines don't start with `export` (e.g., plain assignment).

    4. Add a guarded source line to each shell config that had secrets removed, so secrets remain available at runtime.
       Append to ~/.bashrc, ~/.zshrc, and ~/.profile (if not already present):
       ```bash
       # Load secrets from encrypted chezmoi-managed file
       [ -f ~/.secrets.env ] && source ~/.secrets.env
       ```
       IMPORTANT: Check each file first to avoid adding duplicate source lines. Only add if not already present.

    5. Verify secrets are still available in a new shell after the changes:
       ```bash
       source ~/.bashrc
       echo "GITHUB_PERSONAL_ACCESS_TOKEN is set: $([ -n "$GITHUB_PERSONAL_ACCESS_TOKEN" ] && echo YES || echo NO)"
       echo "EXA_API_KEY is set: $([ -n "$EXA_API_KEY" ] && echo YES || echo NO)"
       echo "GREPTILE_API_KEY is set: $([ -n "$GREPTILE_API_KEY" ] && echo YES || echo NO)"
       ```
       All three must output YES.

    6. Add the secrets file to chezmoi with encryption:
       ```bash
       chezmoi add --encrypt ~/.secrets.env
       ```
       This creates `encrypted_dot_secrets.env.age` in the chezmoi source directory.

    7. Verify the encrypted file exists and the plaintext is NOT in the source:
       ```bash
    ...
    8. Verify round-trip works:
       ```bash
       chezmoi cat ~/.secrets.env
       # Should output the plaintext secrets
       ```

    9. Create `.secrets.env.example` in the chezmoi source directory (NOT encrypted -- this is a template for documentation):
       ```bash
    ...
    10. Re-generate the detect-secrets baseline (now that encrypted files exist):
       ```bash
       cd ~/.local/share/chezmoi
       detect-secrets scan > .secrets.baseline
       ```

    11. Verify pre-commit hooks still pass with the new files:
       ```bash
  ...
  <verify>
    ...
    6. `grep -c "GITHUB_PERSONAL_ACCESS_TOKEN=" ~/.bashrc` -- returns 0 (inline export removed)
    7. `grep -c "EXA_API_KEY=" ~/.bashrc` -- returns 0 (inline export removed)
    8. `grep -c "GREPTILE_API_KEY=" ~/.bashrc` -- returns 0 (inline export removed)
    9. `grep -c "EXA_API_KEY=" ~/.zshrc` -- returns 0 (inline export removed)
    10. `grep -c "GITHUB_PERSONAL_ACCESS_TOKEN=" ~/.profile` -- returns 0 (inline export removed)
    11. `grep "source.*secrets.env" ~/.bashrc` -- shows the sourcing line exists
    12. `grep "source.*secrets.env" ~/.zshrc` -- shows the sourcing line exists
    13. `grep "source.*secrets.env" ~/.profile` -- shows the sourcing line exists
  </verify>
  <done>
    Three secrets (GITHUB_PERSONAL_ACCESS_TOKEN, EXA_API_KEY, GREPTILE_API_KEY) are extracted from shell configs into ~/.secrets.env. The inline export lines are REMOVED from ~/.bashrc, ~/.zshrc, and ~/.profile (satisfying SEC-01). Each shell config now sources ~/.secrets.env via a guarded `[ -f ~/.secrets.env ] && source ~/.secrets.env` line, so secrets remain available at runtime. The secrets file is stored age-encrypted in the chezmoi repo. A .secrets.env.example template with placeholder values exists for documentation. Pre-commit hooks pass with all new files.
  </done>
</task>
...
5. No inline secret exports remain in ~/.bashrc, ~/.zshrc, or ~/.profile
6. All three shell configs contain `source ~/.secrets.env` (guarded)
7. After `source ~/.bashrc`, all three secret env vars are set (sourced from ~/.secrets.env)
</verification>
...
- All inline secret export lines REMOVED from .bashrc, .zshrc, and .profile (SEC-01 satisfied)
- Shell configs source ~/.secrets.env so secrets remain available at runtime
