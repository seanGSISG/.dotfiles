...
Check functions:
```bash
check_pass() { echo "${GREEN}  [PASS]${RESET} $1"; ((PASSED++)); }
check_fail() { echo "${RED}  [FAIL]${RESET} $1"; ((FAILED++)); }
check_warn() { echo "${YELLOW}  [WARN]${RESET} $1"; ((WARNINGS++)); }
```
...
═══════════════════════════════════════════
  ...
  [PASS/FAIL] Environment is [ready/not ready]
```
...
- Running `bash verify.sh` produces output with [PASS]/[FAIL] lines
  </verify>
  <done>
