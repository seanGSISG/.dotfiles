...
must_haves:
  ...
  artifacts:
    ...
    - path: "private_dot_ssh/config"
      provides: "SSH config file managed by chezmoi"
  key_links:
    - from: "bootstrap.sh"
...
IMPORTANT: Do NOT add the SSH config file if it doesn't exist. Only add files that exist in ~/.ssh/.
...
- `grep -q 'install_chezmoi\|apply_chezmoi' bootstrap.sh` — split functions exist
- The main() function calls them in correct order: install_chezmoi -> setup_age_key -> apply_chezmoi
  </verify>
  <done>
