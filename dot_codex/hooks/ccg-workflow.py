...
def build_guidance(task, progress, root):
    ...
    if phase == "analysis":
        if complexity in ("M", "L", "XL"):
            ...
            parts.append("Use the dual-model parallel template in AGENTS.md: --backend antigravity & --backend claude with & + wait.")
    ...
    if phase == "review":
        parts.append("")
        parts.append("⛔ Review phase: call BOTH the frontend model (--backend antigravity) AND Claude (--backend claude) with reviewer role. Two models, not one.")
